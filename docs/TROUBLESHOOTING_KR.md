...
**Context:** 저는 ROS 2 경험이 전혀 없는 상태에서 이 프로젝트에 참여했다. 아래에 정리한 구조 관련 결정은 제가 직접 내린 것이다. 코드 자체는 대부분 AI의 도움을 받아 초안을 작성했고, 이후 제가 검토하고 테스트했다. 테스트는 먼저 Gazebo에서 수행한 뒤, 실물 로봇으로 옮겨 진행했다.
...
이 결정은 제가 단독으로 내린 것이 아니라 팀과 supervisor가 함께 내린 결정이었다. 다만 이후 제 작업 범위에는 직접적인 영향을 주었다.

* Kinova sim-to-real mirroring 작업은 그대로 유지되었다. 이 부분이 제 담당이었고, 프로젝트의 핵심 기술 기여로 남았다.
...
README에서 이 프로젝트를 완전한 물리 통합이 아니라 "시뮬레이션 Ridgeback + 실물 Kinova Gen3"로 설명하는 이유가 여기에 있다. 아래에서 설명하는 mirror node, MoveIt 2 통합, launch 통합, connection workflow는 모두 Plan B 범위에서도 유효했다.
...
**선정 과정.** MoveIt 2 + Gazebo 전체 stack을 실행한 뒤, `rqt_graph`로 live node graph를 확인했다. RViz에서 **Plan**과 **Plan + Execute**를 눌렀을 때 planned motion이 어디에 나타나는지 추적했다. 저는 다음 조건을 만족하는 topic을 찾았다.
...
`/display_planned_path`가 위 세 조건을 모두 만족했다. 이 topic은 MoveIt이 RViz에서 planned path를 visualize할 때 사용하는 topic이며, 저희 setup에서는 `JointTrajectory`로 변환해 양쪽 controller로 전달할 수 있는 practical trajectory source로 사용할 수 있었다.
...
**Credit.** "display-side" topic을 사용하자는 아이디어는 팀 논의 중 한 팀원에게서 나왔다. 저는 이 아이디어를 `rqt_graph`로 검증했고, 이를 바탕으로 bridge / mirror 구조를 구축했다.
...
**결정.** 저는 전체 stack을 ROS 2 Jazzy로 migration하자고 제안했다. 저희 환경에서는 MoveIt 2 자체는 Rolling에서도 문제없이 동작했지만, Clearpath compatibility가 핵심 문제였다. 팀과 논의한 뒤 전체 환경을 Jazzy로 이전했다.
...
반복적인 real-robot testing 중, 같은 trajectory를 실행할 때 simulated arm과 real arm 사이에 작은 timing difference가 보이는 경우가 있었다.
...
이 부분은 future work로 남겼고, 이후 다음 개선 단계에서 정량 측정과 개선을 진행하기 위해 ROS 2 with C++를 공부하기 시작했다.
